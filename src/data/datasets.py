...
def _transform_fast(x, theta):
    """
    Fast rotation augmentation using precomputed rotation matrices
    
    Args:
        x: Input tensor of shape (C, T, V) or (B, C, T, V)
        theta: Maximum rotation angle
    
    Returns:
        Rotated tensor of same shape
    """
    # Handle both (C, T, V) and (B, C, T, V) formats
    if x.dim() == 3:
        x = x.unsqueeze(0)
        squeeze_output = True
    else:
    ...
    # Precompute rotation matrices for all samples at once
    rot_angles = torch.empty(B, 3, device=device, dtype=x.dtype).uniform_(-theta, theta)
    cos_r, sin_r = rot_angles.cos(), rot_angles.sin()
    
    # Create rotation matrices more efficiently
    zeros = torch.zeros(B, 1, device=device, dtype=x.dtype)
    ones = torch.ones(B, 1, device=device, dtype=x.dtype)
    
    # Build rotation matrices
    rx = torch.stack([torch.cat([ones, zeros, zeros], dim=1),
                     torch.cat([zeros, cos_r[:, 0:1], sin_r[:, 0:1]], dim=1),
                     torch.cat([zeros, -sin_r[:, 0:1], cos_r[:, 0:1]], dim=1)], dim=2)  # (B, 3, 3)
    
    ry = torch.stack([torch.cat([cos_r[:, 1:2], zeros, -sin_r[:, 1:2]], dim=1),
                     torch.cat([zeros, ones, zeros], dim=1),
                     torch.cat([sin_r[:, 1:2], zeros, cos_r[:, 1:2]], dim=1)], dim=2)
    
    rz = torch.stack([torch.cat([cos_r[:, 2:3], sin_r[:, 2:3], zeros], dim=1),
                     torch.cat([-sin_r[:, 2:3], cos_r[:, 2:3], zeros], dim=1),
                     torch.cat([zeros, zeros, ones], dim=1)], dim=2)
    
    # Combined rotation: (B, 3, 3)
    rot = torch.bmm(torch.bmm(rz, ry), rx)
    
    # Reshape for batch matrix multiplication
    x = x.permute(0, 2, 3, 1).contiguous()  # (B, T, V, C)
    x = x.view(-1, 3)  # (B*T*V, 3)
    
    # Apply rotation to all points at once
    rot_expanded = rot.unsqueeze(1).repeat(1, T*V, 1, 1).view(-1, 3, 3)  # (B*T*V, 3, 3)
    x = torch.bmm(rot_expanded, x.unsqueeze(-1)).squeeze(-1)  # (B*T*V, 3)
...
def sample_frames_fast(sequence, seg):
    """
    Optimized frame sampling with precomputed indices
    ...
    if num_frames < seg:
        if num_frames > 0:
            # Use np.pad for efficiency
            pad_width = ((0, seg - num_frames), (0, 0))
            processed_seq = np.pad(non_zero_frames, pad_width, mode='edge')
        else:
            # Handle edge case: if non_zero_frames is empty
            processed_seq = np.zeros((seg, sequence.shape[1]), dtype=np.float32)
    else:
        # Precompute indices for equal spacing
        ave_duration = num_frames // seg
        # Use vectorized operations
        base_indices = np.arange(seg) * ave_duration
        offsets = np.random.randint(ave_duration, size=seg)
        offsets = np.clip(base_indices + offsets, 0, num_frames - 1)
...
# Two-person actions removed from NTU datasets
_NTU_TWO_PERSON_ACTIONS = set(range(50, 61)) | set(range(106, 121))  # 1-indexed

# NTU120 single-person actions: 1-49 and 61-105 (94 total)
NTU120_SINGLE_PERSON_ACTIONS = sorted(
    [a for a in range(1, 121) if a not in _NTU_TWO_PERSON_ACTIONS]
)

# Lookup table: 0-indexed original label -> contiguous 0-based index (for NTU120)
# e.g., original 0->0, ..., 48->48, 60->49, 61->50, ..., 104->93
NTU120_ACTION_REMAP = {}
for _new_idx, _orig_action in enumerate(NTU120_SINGLE_PERSON_ACTIONS):
    NTU120_ACTION_REMAP[_orig_action - 1] = _new_idx  # convert to 0-indexed key
...
def get_num_classes(dataset, type='ar'):
    ...
    if type == 'ar':
        # For NTU we drop two-person actions, giving 49 classes instead of 60.
        if dataset in ['ntu', 'ntu_smoke', 'ntu_small']:
            return 49
        # For NTU120 we drop two-person actions (50-60, 106-120), giving 94 classes.
        if dataset == 'ntu120':
            return 94
    ...
    for i in range(len(actors_2plus)):
        ...
        for j in range(i + 1, len(actors_2plus)):
            ...
            if len(common_actions) >= 2:
                ...
                a1, a2 = random.sample(list(common_actions), 2)
...
class Cross_Data(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Apply frame sampling (optimized)
        x1 = sample_frames_fast(x1_raw, self.seg)
        x2 = sample_frames_fast(x2_raw, self.seg)
        y1 = sample_frames_fast(y1_raw, self.seg)
        y2 = sample_frames_fast(y2_raw, self.seg)
        ...
        # Reshape from (T, V*C) to (C, T, V)
        # (T, V*C) -> (T, V, C) -> (C, T, V)
        V = 25
        C = 3
        x1 = x1.reshape(self.seg, V, C).permute(2, 0, 1)  # (C, T, V)
        x2 = x2.reshape(self.seg, V, C).permute(2, 0, 1)
        y1 = y1.reshape(self.seg, V, C).permute(2, 0, 1)
        y2 = y2.reshape(self.seg, V, C).permute(2, 0, 1)

        # Apply augmentation (rotation) if enabled (optimized)
        if self.augment:
            x1 = _transform_fast(x1, self.theta)
            x2 = _transform_fast(x2, self.theta)
            y1 = _transform_fast(y1, self.theta)
            y2 = _transform_fast(y2, self.theta)
...
class ActionRecognitionDataset(Dataset):
    """
    Dataset for action recognition tasks (AR/RI/GC).
    Loads data from raw dictionary and returns (skeleton, label) pairs.

    Args:
        data_dict: dict - Mapping of filename to skeleton data (T, V*C)
        dataset: str - Dataset name ('ntu', 'ntu120', 'etri')
        setting: str - 'cv' for cross-view, 'cs' for cross-subject
        split: str - 'train' or 'test'
        task: str - 'ar' for action recognition, 'ri' for re-identification, 'gc' for gender classification
        seg: int - Number of frames to sample
        augment: bool - Whether to apply data augmentation
        theta: float - Rotation angle for augmentation
    """
    def __init__(self, data_dict, dataset, setting, split='train', task='ar', seg=20, augment=True, theta=0.3,
                 drop_two_person_actions=False):
        ...
        self.theta = theta
        self.drop_two_person_actions = drop_two_person_actions

        # Get train/test split
        train_data, test_data = organize_data(data_dict, setting, dataset)

        # Flatten the organized data into a list of (filename, label) pairs
        # organize_data returns {group_key: summary_dict} where summary_dict has 'pa_map'
        self.samples = []

        def keep_action(a: float) -> bool:
            if not self.drop_two_person_actions:
                return True
            if self.dataset in ('ntu', 'ntu120'):
                # Accept primary actions in either 1-based (1..49) or 0-based (0..48) form.
                # NTU120 two-person actions are already filtered by load_data; this handles NTU60.
                return (1 <= a <= 49) or (0 <= a <= 48)
            return True

        target_split = train_data if split == 'train' else test_data
        for _, summary in target_split.items():
            pa_map = summary['pa_map']  # {(person, action): [filenames]}
            for (person, action), filenames in pa_map.items():
                if task == 'ar' and not keep_action(action):
                    continue
                for filename in filenames:
                    if task == 'ar':
                        ...
                        label = person % 2  # Simple gender mapping
                    else:
                        label = action
                    self.samples.append((filename, label))

        # Get unique labels and create mapping for RI task
        if task == 'ri':
        ...
        # For AR, build a contiguous label map if labels are non-contiguous
        # (e.g., NTU 49 actions after dropping 2-person, NTU120 94 actions)
        if task == 'ar':
            unique_actions = sorted(set([int(label) for _, label in self.samples]))
            # Check if labels are contiguous 0..N-1
            n = len(unique_actions)
            is_contiguous = unique_actions == list(range(n))
            if not is_contiguous:
                self.action_label_map = {old_label: new_label for new_label, old_label in enumerate(unique_actions)}
                self.num_classes = len(self.action_label_map)
            else:
                self.action_label_map = None
                self.num_classes = n
    ...
    def __getitem__(self, index):
        ...
        # Load skeleton data
        skeleton_data = self.data_dict[filename]  # (T, V*C)

        # Normalize layout to (T, V*C). Some archives store as (V*C, T).
        if skeleton_data.ndim == 2:
            if skeleton_data.shape[1] == 75:
                # Already (T, 75)
                pass
            elif skeleton_data.shape[0] == 75:
                # Transpose (75, T) -> (T, 75)
                skeleton_data = skeleton_data.T
            else:
                # Fallback: if neither dim is 75, leave as-is and let sample_frames handle
                pass
        elif skeleton_data.ndim == 3 and skeleton_data.shape[-1] == 3:
            # Convert (T, V, C) -> (T, V*C)
            T, V, C = skeleton_data.shape
            skeleton_data = skeleton_data.reshape(T, V * C)

        # Apply frame sampling (optimized)
        skeleton_data = sample_frames_fast(skeleton_data, self.seg)  # (seg, V*C)

        # Handle multi-person features by keeping the first person only
        if skeleton_data.ndim == 2 and skeleton_data.shape[1] != 75 and skeleton_data.shape[1] % 75 == 0:
            num_people = skeleton_data.shape[1] // 75
            skeleton_data = skeleton_data.reshape(self.seg, num_people, 25, 3)
            skeleton_data = skeleton_data[:, 0, :, :]  # keep person 1
            skeleton_data = skeleton_data.reshape(self.seg, 25 * 3)

        # Convert to torch tensor
        skeleton_data = torch.from_numpy(skeleton_data).float()  # (seg, V*C)

        # Apply rotation augmentation if enabled
        # Note: _transform expects (C, T, V) format, so we need to reshape
        if self.augment:
            # Reshape from (T, V*C) to (C, T, V)
            V = 25
            C = 3
            skeleton_data = skeleton_data.reshape(self.seg, V, C).permute(2, 0, 1)  # (C, T, V)
            skeleton_data = _transform_fast(skeleton_data, self.theta)  # (C, T, V)
            # Reshape back to (T, V*C)
            skeleton_data = skeleton_data.permute(1, 2, 0).reshape(self.seg, -1)  # (T, V*C)

        # Remap label if needed (for RI task)
        if self.label_map is not None:
            label = self.label_map[label]

        # Convert action labels from 1-indexed to 0-indexed
        if self.task == 'ar':
            if getattr(self, 'action_label_map', None):
                label = self.action_label_map[int(label)]
            else:
                label = label - 1
...
class PT_Data(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Apply frame sampling (optimized)
        x = sample_frames_fast(x_raw, self.seg)

        # Convert to torch tensor
        x = torch.from_numpy(x).float()

        # Apply rotation augmentation if enabled (optimized)
        if self.augment:
            x = _transform_fast(x.unsqueeze(0), self.theta).squeeze(0)
...
def get_cross_data(X, dataset, setting, batch_size=32, return_loader=False,
    ...
    if return_loader:
        ...
        val_dl = DataLoader(val_dataset, batch_size=batch_size, shuffle=True, num_workers=1)
