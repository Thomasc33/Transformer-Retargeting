...
class Encoder(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=1, graph=None, graph_args=dict(), in_channels=3, debug=False, dataset='ntu', load_pretrained=True, freeze_layers=True, device='cuda'):
        ...
        if load_pretrained:
            ...
            if debug:
                print("Skipping pretrained encoder weights loading (load_pretrained=False)")
    ...
    def forward(self, x):
        ...
        # Replace rearrange(x, 'n c t v m -> (n m t) v c', m=M)
        ...
        # Replace rearrange(x, '(n m t) v c -> n (m v c) t', n=N, m=M, t=T)
        x = x.view(N, M, T, -1)  # (N, M, T, V*C)
        x = x.permute(0, 2, 1, 3).contiguous()  # (N, T, M, V*C)
        x = x.reshape(N, -1, T)  # (N, M*V*C, T)
        x = self.data_bn(x)

        # Replace rearrange(x, 'n (m v c) t -> (n m) c t v', n=N, m=M, v=V)
        # After data_bn, x has shape (N, C, T) where C = M*V*C_original
        # We need to reshape it properly
        C_current = x.size(1)
        # Calculate expected dimensions
        expected_C = M * V * C
        if C_current != expected_C:
            # If dimensions don't match, we need to reshape differently
            # x is (N, C_current, T), reshape to (N, M, V, C, T) or similar
            # For now, assume it's already in the right format for the encoder layers
            x = x.view(N, -1, T, V)  # (N, C', T, V) where C' might be different
        else:
            x = x.view(N, M, -1, T)  # (N, M, V*C, T)
            x = x.permute(0, 1, 3, 2).contiguous()  # (N, M, T, V*C)
            x = x.reshape(N * M, -1, T)  # (N*M, V*C, T)
