...
import sys
sys.path.append('/users/tcarr23/Transformer-Retargeting')
    ...
    if os.path.exists(checkpoint_path):
        ...
        if isinstance(checkpoint, dict):
            if 'model_state_dict' in checkpoint:
                ...
                # Direct state_dict (possibly from DataParallel)
                model_state_dict = checkpoint
                optimizer_state_dict = None
                start_epoch = 0
                best_loss = float('inf')
                print("Loading direct state_dict - starting from epoch 0")
        else:
            print("Unexpected checkpoint format")
            return 0, float('inf')

        # Load model state dict (handle DDP case and module prefix)
        if isinstance(model, torch.nn.parallel.DistributedDataParallel):
            # Remove 'module.' prefix if present for DDP loading
            if any(k.startswith('module.') for k in model_state_dict.keys()):
                model_state_dict = {k.replace('module.', ''): v for k, v in model_state_dict.items()}
            model.module.load_state_dict(model_state_dict)
        else:
            # Handle module prefix for non-DDP model
            if any(k.startswith('module.') for k in model_state_dict.keys()):
                model_state_dict = {k.replace('module.', ''): v for k, v in model_state_dict.items()}
...
def train_model(model, optimizer, train_loader, test_loader, loss_fn, num_epochs, device, rank, world_size,
    ...
    # Start profiling
    if is_main_process and global_profiler is not None:
...
def main():
    ...
    model = Model(num_class=120 if dataset == 'ntu120' else 60,
                  num_point=V, num_person=M, graph='src.graph.ntu_rgb_d.Graph',
