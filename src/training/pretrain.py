...
from src.data import load_data, process_mlm, Masked_AE_Data
from src.model.encoder import Encoder, pre_process
from src.model.mlm_decoder import MLMDecoder, post_process
import numpy as np
...
class SkeletonAutoEncoder(nn.Module):
    ...
    def __init__(self, dataset, seq_len):
        ...
        self.encoder = Encoder(
            num_class=94 if dataset == 'ntu120' else (49 if dataset in ('ntu', 'ntu_smoke', 'ntu_small') else 55),
            num_point=25,
            num_person=1,
    ...
    if rank == 0 and args.enable_linear_probe:
        ...
        if not best_encoder_path_for_probe or not os.path.exists(best_encoder_path_for_probe):
            ...
            probe_encoder_instance = Encoder(
                num_class=94 if args.dataset == 'ntu120' else (49 if args.dataset in ('ntu', 'ntu_smoke', 'ntu_small') else 55),
                num_point=25, # Assuming NTU default
                num_person=1, # Assuming single person skeletons for pretraining
                graph='src.graph.ntu_rgb_d.Graph', # Assuming NTU default
                graph_args={'labeling_mode': 'spatial'},
                in_channels=3,
