...
def eval_skeleton_sgn(paired_test, anonymizer_model, dataset_name, ar_model_weights, ri_model_weights, args):
    """Modified SGN evaluation function to handle 5-crop test augmentation"""
    from model.sgn import SGN
    from data import get_num_classes
    
    # Initialize models and load weights (unchanged)
    ...
    # Process data through anonymizer (unchanged)
    anonymized_data = []
    ...
    all_ri_preds = []
    all_ri_labels = []
    all_ar_logits = []  # Store all averaged logits for top-3 calculation
    ...
    for item in tqdm(anonymized_data, desc="Evaluating SGN"):
        ...
        try:
            ...
            processed_crops = sgn_preprocess_single_skeleton(skel_np, seg=20, dataset=dataset_name)
            
            # Process all 5 crops
            ar_outputs = []
            ri_outputs = []
            
            for crop_idx in range(processed_crops.shape[0]):
                crop_tensor = torch.from_numpy(
                    processed_crops[crop_idx:crop_idx+1]).float().to(args.device)
                
                with torch.no_grad():
                    ar_out = sgn_ar(crop_tensor)
                    ri_out = sgn_ri(crop_tensor)
                    ar_outputs.append(ar_out)
                    ri_outputs.append(ri_out)
            
            # Average predictions across 5 crops
            ar_output = torch.mean(torch.stack(ar_outputs), dim=0)
            ri_output = torch.mean(torch.stack(ri_outputs), dim=0)
            
            # Store logits for top-k calculation
            all_ar_logits.append(ar_output.cpu().numpy())
    ...
    if len(all_ar_preds) > 0:
        ...
        # Fixed top-3 calculation that works with stored logits
        top3_correct = 0
        for i, label in enumerate(all_ar_labels):
            logits = all_ar_logits[i][0]  # Get the logits for this sample
            top3_indices = np.argsort(logits)[-3:]  # Get indices of top 3 predictions
            if label in top3_indices:
                top3_correct += 1
        ar_top3 = 100.0 * top3_correct / len(all_ar_labels)
    else:
        ar_f1 = ar_prec = ar_rec = ar_top3 = 0
    
    # Output metrics
    print(f"[SGN] Action Recognition Results:")
    print(f"  Accuracy: {ar_acc:.2f}%   F1: {ar_f1*100:.2f}%   Prec: {ar_prec*100:.2f}%   Rec: {ar_rec*100:.2f}%   Top3: {ar_top3:.2f}%")
    ...
    print(f"[SGN] Retargeted Actor: {ri_ret_correct}/{test_size} ({ri_ret_acc:.2f}%)")
    print(f"[SGN] Original  Actor: {ri_orig_correct}/{test_size} ({ri_orig_acc:.2f}%)")
    print(f"[SGN] Neither:          {ri_neither}/{test_size} ({ri_neither_acc:.2f}%)")
