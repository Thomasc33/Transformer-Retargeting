...
def seq_translation(skes_joints):
    for idx, ske_joints in enumerate(skes_joints):
        ...
        if i >= num_frames:  # all frames are zero (empty skeleton from denoising)
            continue
...
def frame_translation(skes_joints, skes_name, frames_cnt):
    ...
    for idx, ske_joints in enumerate(skes_joints):
        ...
        for f in range(num_frames):
            if dist[f] < 1e-8:  # skip zero/degenerate frames
                continue
...
if __name__ == '__main__':
    ...
    # Note: skes_joints may have fewer entries than stats files if some CSVs
    # were skipped during raw extraction (missing/corrupt files). The split_dataset
    # function assumes matching sizes. Skip CS/CV split for now — not needed by
    # DisentangledTMR pipeline (it does its own paired data generation).
    #
    # evaluations = ['CS', 'CV']
    # for evaluation in evaluations:
    #     split_dataset(skes_joints, label, performer, camera, evaluation, save_path)

    # Save as dict pkl for DisentangledTMR pipeline
    # Map each skeleton name to its aligned joint sequence
    print('Aligned joints shape:', skes_joints.shape)
    print('Available names:', skes_name.shape[0])

    # Build name-to-joints mapping using only the entries that survived preprocessing
    # skes_joints comes from denoised pickle (may be < skes_name if raw extraction skipped some)
    num_joints = skes_joints.shape[0]
    if num_joints == skes_name.shape[0]:
        X = {skes_name[i].decode('utf-8'): skes_joints[i] for i in range(num_joints)}
    else:
        print(f'WARNING: {num_joints} joints vs {skes_name.shape[0]} names — using index-based mapping')
        X = {skes_name[i].decode('utf-8'): skes_joints[i] for i in range(min(num_joints, skes_name.shape[0]))}

    with open('etri.pkl', 'wb') as f:
        pickle.dump(X, f)
    print('Saved etri.pkl with %d samples' % len(X))