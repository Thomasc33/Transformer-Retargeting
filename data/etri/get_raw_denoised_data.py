...
def get_two_actors_points(bodies_data):
    ...
    """
    ske_name = bodies_data['name']
    # ETRI format: A052P038G002C001 — extract action label from first field
    try:
        label = int(ske_name[1:4])  # e.g., 'A052...' -> 52
    ...
    if len(bodies_data) == 0:  # All bodies removed by denoising
        num_joints = 75  # 25 joints * 3 coords
        joints = np.zeros((num_frames, num_joints), dtype=np.float32)
        colors = np.ones((num_frames, 1, 25, 2), dtype=np.float32) * np.nan
        bodies_info += 'All bodies removed by denoising'
        return joints, colors
    elif len(bodies_data) == 1:  # Only left one actor after denoising
