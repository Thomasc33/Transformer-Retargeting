...
def setup_3d_axis(ax, skeleton_data, title=""):
    ...
    y_min, y_max = all_data[:, :, 1].min() - padding, all_data[:, :, 1].max() + padding
    z_min, z_max = all_data[:, :, 2].min() - padding, all_data[:, :, 2].max() + padding
...
def create_comparison_gif(original_skeleton, reconstructed_skeleton, output_path,
    ...
    try:
        ...
        for frame_idx in tqdm(range(num_frames), desc="Generating frames"):
            ...
            plt.suptitle(' | '.join(title_parts), fontsize=16, fontweight='bold')
        ...
        for batch_idx, batch_content in enumerate(data_loader):
            ...
            for i in range(min(x_data.shape[0], num_samples - sample_count)):
                try:
                    ...
                    comparison_path = os.path.join(output_dir, f'mlm_comparison_sample_{sample_count+1}.gif')
                    ...
                    # Create overlay GIF
                    overlay_path = os.path.join(output_dir, f'mlm_overlay_sample_{sample_count+1}.gif')
                    create_overlay_gif(
