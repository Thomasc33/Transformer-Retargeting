...
def eval_skeleton_sgn(
    ...
    print("[SGN] Evaluating with snippet-based approach...")

    AR_Model = import_class('model.sgn.SGN')
    sgn_ar = AR_Model(
        num_classes=datasets[dataset_name]['num_class'],
        dataset=dataset_name,
        seg=20
    ).cuda(args.device)
    sd_ar = torch.load(ar_model_weights, map_location='cpu')
    if 'state_dict' in sd_ar:
        sd_ar = sd_ar['state_dict']
    sd_ar = {k.replace('module.', ''): v for k, v in sd_ar.items()}
    sgn_ar.load_state_dict(sd_ar)
    sgn_ar.eval()

    RI_Model = import_class('model.sgn.SGN')
    sgn_ri = RI_Model(
        num_classes=datasets[dataset_name]['num_actor'],
        dataset=dataset_name,
        seg=20
    ).cuda(args.device)
    sd_ri = torch.load(ri_model_weights, map_location='cpu')
    if 'state_dict' in sd_ri:
        sd_ri = sd_ri['state_dict']
    sd_ri = {k.replace('module.', ''): v for k, v in sd_ri.items()}
    sgn_ri.load_state_dict(sd_ri)
    sgn_ri.eval()
    ...
    def anonymize_batch(batch):
        if args.model_type == 'raw':
            ...
            return get_anonymized_paired_transformer(batch, anonymizer_model, prep_data)
        else:
            return get_anonymized_paired_dmr_pmr(batch, anonymizer_model, T=args.T)

    # 1) Collect all anonymized skeletons
    for batch in tqdm(paired_test, desc="SGN anonymize"):
        anonymized_batch = anonymize_batch(batch)

        for item in anonymized_batch:
            anonymized_skel = item['skeleton']  
            gt_skel        = item['gt_skeleton'] 
            ...
            t_in  = gt_skel.shape[0]
            Tm = min(t_out, t_in)
            m_ = F.mse_loss(anonymized_skel[:Tm], gt_skel[:Tm])
            mse_sum += m_.item()

            X_300_150 = anonymizer_to_sgn(anonymized_skel, max_frames=300)

            anonymized_list.append({
                'X': X_300_150,        
                'action_label': action_label,
                'ret_actor': ret_actor,
                'orig_actor': orig_actor
            })
            total_samples += 1

    # 2) Build arrays for snippet test (AR / RI)
    num_actions = datasets[dataset_name]['num_class']
    num_actors  = datasets[dataset_name]['num_actor']

    N_ = len(anonymized_list)
    test_x_ar = np.zeros((N_, 300, 150), dtype=np.float32)
    test_y_ar = np.zeros((N_, num_actions), dtype=np.float32)
    test_x_ri = np.zeros((N_, 300, 150), dtype=np.float32)
    test_y_ri = np.zeros((N_, num_actors), dtype=np.float32)

    for i, item in enumerate(anonymized_list):
        x_cur = item['X']

        # If x_cur is (300,75), we do final duplication => (300,150) if you want 2-person BN.
        if x_cur.shape[1] == 75:
            buffer = np.zeros_like(x_cur)
            temp_ = np.concatenate([x_cur, buffer], axis=1) # shape => (300,150)
            x_cur = temp_

        test_x_ar[i] = x_cur
        aidx = int(item['action_label'].item())
        test_y_ar[i, aidx] = 1.0

        test_x_ri[i] = x_cur
        # ret_actor => label for classification
        a2 = int(item['ret_actor'].item())
        test_y_ri[i, a2] = 1.0

    train_x_dummy = np.zeros((1,300,150), dtype=np.float32)
    train_y_dummy = np.zeros((1, num_actions), dtype=np.float32)
    val_x_dummy   = np.zeros((1,300,150), dtype=np.float32)
    val_y_dummy   = np.zeros((1, num_actions), dtype=np.float32)

    # 3) Evaluate AR with snippet test:
    acc_ar, f1_ar, prec_ar, rec_ar, topk_ar = run_sgn_eval_snippet(
        train_x_dummy, train_y_dummy,
        test_x_ar, test_y_ar,
        val_x_dummy, val_y_dummy,
    ...
    # 4) Evaluate RI with snippet test:
    acc_ri, f1_ri, prec_ri, rec_ri, topk_ri = run_sgn_eval_snippet(
        train_x_dummy, train_y_dummy,
        test_x_ri, test_y_ri,
        val_x_dummy, val_y_dummy,
        dataset_name=dataset_name,
    ...
    # 5) ret/orig logic (manually compute predicted actor => compare with ret/orig)
    sgn_ri.eval()
    ret_correct = 0
    orig_correct = 0
    neither_correct = 0
    ...
    idx_start = 0
    while idx_start < N_:
        idx_end = min(idx_start + chunk_size, N_)
        batch_x = test_x_ri[idx_start:idx_end]
        batch_ret = [anonymized_list[i]['ret_actor'] for i in range(idx_start, idx_end)]
        batch_orig = [anonymized_list[i]['orig_actor'] for i in range(idx_start, idx_end)]

        bx_t = torch.tensor(batch_x, dtype=torch.float32, device=args.device)
        # Split into segments of up to 75 frames along dim=1
        segment_outputs = []
        max_frames = 20
        total_frames = bx_t.size(1)
        num_segments = (total_frames + max_frames - 1) // max_frames

        with torch.no_grad():
            for seg_idx in range(num_segments):
                start = seg_idx * max_frames
                end = min(start + max_frames, total_frames)
                seg = bx_t[:, start:end, :75]
                out_seg = sgn_ri(seg)
                if out_seg.dim() == 3:
                    out_seg = out_seg.mean(dim=1)
                segment_outputs.append(out_seg)

            out_ri = torch.mean(torch.stack(segment_outputs, dim=0), dim=0)

        pred_ri = out_ri.argmax(dim=1).cpu().numpy()
        for i_inbatch, predval in enumerate(pred_ri):
            if predval == batch_ret[i_inbatch]:
                ret_correct += 1
            elif predval == batch_orig[i_inbatch]:
                orig_correct += 1
            else:
                neither_correct += 1
    ...
    total_ri = N_
    ret_pct = 100.0 * ret_correct / total_ri if total_ri else 0
    orig_pct = 100.0 * orig_correct / total_ri if total_ri else 0
    neither_pct = 100.0 * neither_correct / total_ri if total_ri else 0

    avg_mse = mse_sum / total_samples if total_samples else 0

    print(f"[SGN] Action Recognition Results:")
    print(f"  Accuracy:  {acc_ar:.2f}%   F1: {f1_ar*100:.2f}%   Prec: {prec_ar*100:.2f}%   Rec: {rec_ar*100:.2f}%   Top3: {topk_ar:.2f}%")
    print(f"[SGN] Re-identification => ret_actor as label:")
    print(f"  Accuracy:  {acc_ri:.2f}%   F1: {f1_ri*100:.2f}%   Prec: {prec_ri*100:.2f}%   Rec: {rec_ri*100:.2f}%   Top3: {topk_ri:.2f}%")
    print(f"[SGN] Retargeted Actor: {ret_correct}/{total_ri} ({ret_pct:.2f}%)")
    print(f"[SGN] Original  Actor: {orig_correct}/{total_ri} ({orig_pct:.2f}%)")
    print(f"[SGN] Neither:          {neither_correct}/{total_ri} ({neither_pct:.2f}%)")
