...
class Spatial_MixFormer(nn.Module):
    def __init__(self, in_channels, out_channels, A, groups=8, coff_embedding=4, num_subset=3,t_stride=1,t_padding=0,t_dilation=1,bias=True,first=False,residual=True):
        ...
        # Store as a regular tensor instead of Variable for better device handling
        self.register_buffer('A_SE', torch.from_numpy(np.reshape(A.astype(np.float32),[3,1,25,25]).repeat(groups,axis=1)))
        ...
        nn.init.normal_(self.fc.weight, 0, math.sqrt(2. / 25))
        self.bn = nn.BatchNorm2d(out_channels)
        self.soft = nn.Softmax(-1)
        self.relu = nn.ReLU()
    ...
    def forward(self, x0):
        ...
        device = x0.device
        A = self.A_SE.to(device) + self.A_GEME.to(device)
        norm_learn_A = A.repeat(1,self.out_channels//self.groups,1,1)
        A_final=torch.zeros([N,self.num_subset,self.out_channels,25,25],dtype=torch.float,device=device).detach()
        m = x0
        m = self.conv(m)
        n, kc, t, v = m.size()
