...
def load_config(config_path: str = "evaluation_suite/configs/experiments.yaml") -> Dict[str, Any]:
    """Load experiment configuration from YAML file. If yaml is unavailable, return a minimal stub."""
    if yaml is None:
        logging.warning("PyYAML not available; using minimal config stub. Install pyyaml for full functionality.")
        return {
            'experiment_sets': {
                'critical': {'experiments': ['baseline_comparison', 'physical_plausibility']},
