...
try:
    from eval.suite.analysis.visualizer import ComprehensiveVisualizer
except Exception:
...
class ComprehensiveEvaluator:
    ...
    def __init__(self, config: Dict[str, Any], results_dir: str = "results"):
        ...
        self.results_manager = ResultsManager(base_results_dir=str(self.results_dir))
    ...
    def run_experiment(self, experiment_name: str, experiment_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # Create experiment directory via ResultsManager
            exp_dir, exp_id = self.results_manager.init_experiment(experiment_name)
            self.current_exp_dir = exp_dir
            self.experiment_id = exp_id

            # If running on Slurm, record job info
            slurm_job_id = os.environ.get('SLURM_JOB_ID')
            slurm_script = os.environ.get('SLURM_JOB_NAME')  # script path not directly available
            if slurm_job_id:
                self.results_manager.record_slurm_info(exp_dir, slurm_job_id, slurm_script)
            ...
            self.results_manager.save_results_bundle(experiment_results, exp_dir)

            # Best-effort: regenerate dashboard after each experiment run
            try:
                from eval.suite.generate_dashboard import main as regenerate
                regenerate()
            except Exception as dash_e:
                self.logger.warning(f"Dashboard regeneration skipped/failed: {dash_e}")
            ...
            # Save failure bundle so dashboard can reflect missing items
            try:
                if self.current_exp_dir is None:
                    self.current_exp_dir, self.experiment_id = self.results_manager.init_experiment(experiment_name)
                failure = {
                    'experiment_name': experiment_name,
                    'experiment_id': self.experiment_id,
                    'config': experiment_config,
                ...
                self.results_manager.save_results_bundle(failure, self.current_exp_dir)
            except Exception as save_e:
                self.logger.warning(f"Could not save failure bundle: {save_e}")
            # Re-raise to preserve non-zero exit on failure
            raise
    ...
    def evaluate_privacy_utility(self, model: Any, data_loader: Any, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Track which anonymizer/eval/task combos we've already run to avoid duplicates across datasets
        ...
        for model_name, model_cfg in models_config.items():
            for eval_model_name, eval_cfg in eval_models_config.items():
                key = f"{model_name}_{eval_model_name}_{eval_cfg.get('task', 'ar')}"
                for data_name, data_cfg in data_config.items():

                    # Create output directory for this combination inside unified artifacts dir
                    combo_dir = base_artifacts_dir / f"{model_name}_{eval_model_name}_{data_name}"
                    combo_dir.mkdir(parents=True, exist_ok=True)
                    output_dir = str(combo_dir)

                    # Only execute eval_model once per (anonymizer, eval_model, task) and reuse results path
                    if key not in seen_keys:
                        run_result = self.eval_integration.run_eval_model(
                        ...
                        seen_keys.add(key)
                        results[f"{model_name}_{eval_model_name}_{data_name}"] = run_result
                    else:
                        # If already run for a different dataset config, just record the path/marker
                        results[f"{model_name}_{eval_model_name}_{data_name}"] = {
                            'status': 'skipped_duplicate_combo',
                            'reuse_from': key,
                            'output_dir': output_dir
                        }
