...
Restored from git history (commit 8acd08e) and notebook (PMR.ipynb) for loading
pretrained checkpoints. Uses 2D convolution encoders with encoded_channels = (128, 16).
...
# PMR architecture parameters
T = 75
encoded_channels = (128, 16)


class Encoder2D(nn.Module):
    """PMR 2D Encoder. Input: (B, T, 25, 3). Output: (B, 128, 16)."""
...
class Decoder1D(nn.Module):
    """PMR 1D Decoder. Input: (B, 256, 16). Output: (B, T, 75)."""
...
class AutoEncoder(nn.Module):
    """
    PMR AutoEncoder (matches notebook class name for checkpoint compatibility).
    ...
    def __init__(self, use_adv=False):
        ...
        # Core autoencoder components (2D encoders, 1D decoder)
